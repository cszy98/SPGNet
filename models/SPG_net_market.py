...
class DualUnetGenerator_SEAN(nn.Module):
    ...
    def __init__(self, pose_nc, appearance_nc, output_nc, aux_output_nc=[], nf=32, max_nf=128, num_scales=7,
        ...
        for l in range(num_scales):
            ...
            upsample_norm = ACE(our_norm_type, c_in, 3, ACE_Name='Block' + '_ACE_0', status='train' if self.is_train else 'test',
                                spade_params=[our_norm_type, c_in, self.nc_cihp], use_rgb=True)
            ...
            for l in range(self.num_scales):
                for i in range(self.n_residual_blocks):
                    ...
                    if use_fw and l < self.num_warp_scales:
                        ...
                        hidden_a.append(x_a)
                # x_a = self.__getattr__('enca_%d_downsample_norm' % l)(x_a, s_seg)
