...
class FlowRegressionModel(BaseModel):
    ...
    def forward(self):
        # if self.opt.usedismap != '':
        #     input = [self.get_input_tensor(self.opt.input_type1, '1'),self.get_input_tensor('dissrc'),self.get_input_tensor(self.opt.input_type2, '2'),self.get_input_tensor('disdst')]
        # else:
    ...
    def get_input_dim(self, input_type):
        ...
        for item in input_items:
            if item == 'img':
                ...
                dim += 3
            elif item == 'dissrc' or item=='disdst':
                dim += 12
            else:
    ...
    def get_input_tensor(self, input_type, index='1'):
        ...
        for item in input_items:
            if item == 'img':
                ...
                tensor.append(self.input['dissrc'])
            elif item == 'disdst':
                tensor.append(self.input['disdst'])
            else:
                raise Exception('invalid input type %s'%item)
