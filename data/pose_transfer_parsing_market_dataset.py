...
class PoseTransferParsingPredDataset(BaseDataset):
    def name(self):
        return 'PoseTransferParsingPredDataset'
    ...
    def initialize(self, opt, split):
        ...
        if split=='test_small':
            self.id_list=data_split['test'][:2000]
        else:
            self.id_list = data_split[split]
    ...
    def read_seg_cihp(self, sid1, sid2):
        fn = os.path.join(self.seg_cihp_dir, sid1+'___'+sid2+'.png')

        seg = cv2.imread(fn, cv2.IMREAD_GRAYSCALE)#.astype(np.float32)[..., np.newaxis]
        seg = seg[:,64:192]
        seg = cv2.resize(seg,(64,128),interpolation=cv2.INTER_NEAREST)
        seg = seg.astype(np.float32)[..., np.newaxis]
        return seg
    ...
    def __getitem__(self, index):
        ...
        seg_cihp_label_1 = self.read_seg_cihp(sid1, sid1)
        seg_cihp_label_2 = self.read_seg_cihp(sid1, sid2)
        ...
        if use_augmentation:
            ...
            seg_cihp_label_2 = \
            cv2.warpAffine(seg_cihp_label_2, M, dsize=(w, h), flags=cv2.INTER_NEAREST, borderMode=cv2.BORDER_REPLICATE)[
                ..., np.newaxis]
            corr_2to1 = cv2.warpAffine(corr_2to1, M, dsize=(w, h), flags=cv2.INTER_LINEAR,
                                       borderMode=cv2.BORDER_REPLICATE)
            vis_2 = cv2.warpAffine(vis_2, M, dsize=(w, h), flags=cv2.INTER_NEAREST, borderMode=cv2.BORDER_REPLICATE)[...,np.newaxis]
        ...
        data = {
            ...
            'seg_cihp_1': seg_cihp_1,
            'seg_cihp_2': seg_cihp_2,
